...
async function fetchGitHubRepos(username: string): Promise<GitHubRepo[]> {
  try {
    ...
    // Priority repos to fill remaining slots if not enough starred repos
    const priorityRepoNames = ["next-app-i18n", "ig-clone-app"];

    // Get starred repos (repos with stars > 0) sorted by star count
    ...
    // Start with starred repos
    const selectedRepos: GitHubRepo[] = [...starredRepos];

    // If we need more repos, add priority repos in order
    ...
    // If still need more repos, add other repos by recent activity
    if (selectedRepos.length < 4) {
      const alreadySelectedNames = selectedRepos.map((repo) => repo.name);
  ...
  const fallbackRepos = [
    {
      ...
      html_url: "https://github.com/dorjitshering/portfolio-website",
      stargazers_count: 12,
      language: "TypeScript",
    },
    {
      ...
      description:
        "A full-featured task management application with drag-and-drop functionality",
      html_url: "https://github.com/dorjitshering/react-task-manager",
      stargazers_count: 8,
      language: "JavaScript",
    },
    {
      ...
      description:
        "E-commerce platform with cart, payments, and admin dashboard",
      html_url: "https://github.com/dorjitshering/nextjs-ecommerce",
      stargazers_count: 15,
      language: "TypeScript",
    },
    {
      ...
      description:
        "Collection of useful TypeScript utility functions and helpers",
      html_url: "https://github.com/dorjitshering/typescript-utils",
      stargazers_count: 6,
      language: "TypeScript",
    },
  ];
      ...
      <div className="max-w-[700px] mx-auto px-6">
        ...
        {repos.length === 0 && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
            Unable to fetch live GitHub data. Showing sample repositories.
          </div>
        )}
